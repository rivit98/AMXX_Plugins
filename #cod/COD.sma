...
public plugin_init() 
{
	...
	cvar_doswiadczenie_za_zabojstwo = register_cvar("cod_killxp", "15");
	cvar_doswiadczenie_za_obrazenia = register_cvar("cod_damagexp", "13"); // ilosc doswiadczenia za 20 obrazen 
	...
	cvar_limit_poziomu = register_cvar("cod_maxlevel", "301"); 
...
{	
	precache_sound("pbsound/select.wav");
	precache_sound("pbsound/start.wav");
	precache_sound("pbsound/start2.wav");
	precache_sound("pbsound/levelup.wav");
}
...
public PoczatekRundy()	
{
	...
	for(new id=0;id<=32;id++)
	{
		...
		{
			case 1: client_cmd(id, "spk pbsound/start2");
			case 2: client_cmd(id, "spk pbsound/start");
		}
	}
}
...
public WygranaRunda(const Team[])
{
	...
	for (new i=0; i<playerCount; i++) 
	{
		...
		client_print(id, print_chat, "[Paintball] Dostales %i doswiadczenia za wygrana runde.", doswiadczenie_za_wygrana);
		SprawdzPoziom(id);
	}
}
...
public OpisPerkow(id)
{
	...
	client_cmd(id, "spk pbsound/select");
}
...
public WybierzKlase_Handle(id, menu, item)
{
	...
	if(klasa_gracza[id])
		client_print(id, print_chat, "[Paintball] Klasa zostanie zmieniona w nastepnej rundzie.");
	else
	{
		UstawNowaKlase(id);
		DajBronie(id);
	...
	{ 
		...
		{	
			if(inteligencja_gracza[id] < limit_poziomu/2)
			{
				...
				client_print(id, print_chat, "[Paintball] Maxymalny poziom inteligencji osiagniety");
		...
		{	
			if(zdrowie_gracza[id] < limit_poziomu/2)
			{
				...
				client_print(id, print_chat, "[Paintball] Maxymalny poziom sily osiagniety");
		}
		case 2: 
		{	
			if(wytrzymalosc_gracza[id] < limit_poziomu/2)
			{
				...
				client_print(id, print_chat, "[Paintball] Maxymalny poziom zrecznosci osiagniety");
		...
		{	
			if(kondycja_gracza[id] < limit_poziomu/2)
			{
				...
				client_print(id, print_chat, "[Paintball] Maxymalny poziom kondycji osiagniety");
		}
	}
...
{	
	client_print(id, print_chat, "[Paintball] Umiejetnosci zostana zresetowane.");
	client_cmd(id, "spk QTM_CodMod/select");
...
public WyrzucPerk(id)
{
	if(perk_gracza[id])
	{
		client_print(id, print_chat, "[Paintball] Wyrzuciles %s.", nazwy_perkow[perk_gracza[id]]);
		UstawPerk(id, 0, 0, 0);
	}
	else
		client_print(id, print_chat, "[Paintball] Nie masz zadnego perku.");
}
...
{	
	...
	else if(zdobyl_poziom)
	{
		...
		client_cmd(id, "spk pbsound/levelup");
	}
...
public PokazInformacje(id) 
{
	...
	if(!is_user_alive(id))
	{
		...
		set_hudmessage(245, 163, 7, 0.6, -1.0, 0, 0.0, 0.3, 0.0, 0.0, 2);
	...
	set_hudmessage(245, 163, 7, 0.02, 0.23, 0, 0.0, 0.3, 0.0, 0.0);
...
public PokazReklame(id)
{
	...
	client_print(id, print_chat, "[Paintball] Witaj w Modyfikacji Paintball + Klasy + EXP");
	client_print(id, print_chat, "[Paintball] W celu uzyskania informacji o komendach napisz /pomoc.");
}
...
public UstawPerk(id, perk, wartosc, pokaz_info)
{
	...
	if(pokaz_info && perk_gracza[id]) 
		client_print(id, print_chat, "[Paintball] Zdobyles %s.", nazwy_perkow[perk_gracza[id]]);
