...
native set_user_bm(id, amount);
native get_user_bm(id);

new g_iUsluga;

#define NAZWA_DL "Bezlitosne Monety" 				//Nazwa uslugi wyswietlana w menu
#define NAZWA_KR "monety" 				//nazwa uslugi uzywana w logach, jako okienko MOTD itd.


new const g_szJednostkaIlosci[][][] = {
	{ "500", "500 Bezlitosnych monet" },
	{ "1000", "1,000 Bezlitosnych monet" },
	{ "2000", "2,000 Bezlitosnych monet" }
...
public ss_buy_service_post(id, iUsluga, iJednostkaIlosci)
{
	...
	set_user_bm(id, get_user_bm(id) + str_to_num(g_szJednostkaIlosci[iJednostkaIlosci][0]));
